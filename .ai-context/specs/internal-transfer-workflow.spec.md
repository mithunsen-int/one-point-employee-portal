...
## Status
In QA
*(Reopened `In QA` → `In Development` 2026-09-21 for `HOTFIX-2026-0921-stakeholder-lost-view-after-action` (`AC19` third amendment, `AC21`–`AC23` one amendment each); returned to `In QA` same day once the hotfix's Gate 1 re-confirmation and Gate 2 verdict (Test Reviewer, 2026-09-21) were both recorded.)*
...
19. internal-transfer-workflow.AC19 — Given the requesting Employee, when they request their own request's status/history/pending stakeholders, then the system returns that data; given the currently-relevant assigned stakeholder for that specific request (Manager/HR/Payroll/IT/Facilities, using the identical per-role eligibility rule already defined by `AC21`–`AC23`), the system also returns that data; given a Manager/HR/Payroll/IT/Facilities user who has personally, previously acted on this specific request (an `AuditLog` entry exists for this request with their own `actorId`), the system also returns that data — permanently, regardless of current status, read-only (no action panel is implied or granted by this clause; each action endpoint's own eligibility, unchanged, is what actually gates any action); given any other non-Admin role, or Admin, the system rejects with 403.
    ...
    *(Amended again 2026-09-21, via `HOTFIX-2026-0921-stakeholder-lost-view-after-action` — the "currently-relevant" carve-out above only ever covered a stakeholder who has **not yet** acted; the moment they did act, status moved on and they lost all view access to a request they were legitimately involved in, surfacing as a real bug reported from actual usage ("Something went wrong loading your request" immediately after approving). Added a historical-actor clause, scoped to the specific person via the audit trail (not their whole role) — least-privilege, and precisely what the audit trail already proves. Explicitly read-only: does not grant or imply any action eligibility, which stays exactly as each action endpoint already independently enforces. Gate 1 re-confirmed 2026-09-21 — see `gate-reviews/gate1-review-internal-transfer-workflow.md`.)*
...
21. internal-transfer-workflow.AC21 — Given a Manager, when they request `API10`, then the system returns only requests currently assigned to them (`assignedManagerId`) with status `Pending: Manager`; additionally, given that same Manager has personally, previously acted on a specific request (an `AuditLog` entry exists for that request with their own `actorId`), the system also includes that request — permanently, regardless of current status, read-only, the identical historical-actor carve-out `AC19` already defines for the detail view.
    *(Amended 2026-09-21, via `HOTFIX-2026-0921-stakeholder-lost-view-after-action` — this list is `AC2c`'s only navigation path into the detail screen, so `AC19`'s historical-actor carve-out for `API02` was reachable in theory but undiscoverable in practice: a stakeholder who'd acted on a request could no longer find it in "My Requests" to click into it. Gate 1 re-confirmed 2026-09-21 — see `gate-reviews/gate1-review-internal-transfer-workflow.md`.)*
22. internal-transfer-workflow.AC22 — Given an HR user, when they request `API10`, then the system returns every request in `Pending: HR`, plus every request in `Pending: Transfer`; additionally, given that same HR user has personally, previously acted on a specific request (an `AuditLog` entry exists for that request with their own `actorId`), the system also includes that request — permanently, regardless of current status, read-only, the identical historical-actor carve-out `AC19` already defines for the detail view.
    *(Amended 2026-09-20 — previously "every request in `Pending: Payroll, IT, Facilities` whose Payroll/IT/Facilities tasks are all `Completed`"; simplified to the new status directly, per `AC25`. Gate 1 re-confirmed 2026-09-20.)*
    *(Amended again 2026-09-21, via `HOTFIX-2026-0921-stakeholder-lost-view-after-action` — same historical-actor gap as `AC21`'s amendment above. Gate 1 re-confirmed 2026-09-21 — see `gate-reviews/gate1-review-internal-transfer-workflow.md`.)*
23. internal-transfer-workflow.AC23 — Given a Payroll, IT, or Facilities user, when they request `API10`, then the system returns only requests in `Pending: Payroll, IT, Facilities` where that role's own task status is still `Pending`; additionally, given that same user has personally, previously acted on a specific request (an `AuditLog` entry exists for that request with their own `actorId`), the system also includes that request — permanently, regardless of current status, read-only, the identical historical-actor carve-out `AC19` already defines for the detail view.
    *(Amended 2026-09-21, via `HOTFIX-2026-0921-stakeholder-lost-view-after-action` — same historical-actor gap as `AC21`'s amendment above. Gate 1 re-confirmed 2026-09-21 — see `gate-reviews/gate1-review-internal-transfer-workflow.md`.)*
...
| internal-transfer-workflow.UT21a | AC21 | Manager who approved an `AuditLog`-recorded request now `Completed` requests `API10` | That request is included, read-only |
...
| internal-transfer-workflow.UT22a | AC22 | HR who performed the final mapping on an `AuditLog`-recorded request now `Completed` requests `API10` | That request is included, read-only |
...
| internal-transfer-workflow.UT23a | AC23 | Payroll who completed their task on an `AuditLog`-recorded request now `Completed` requests `API10` | That request is included, read-only |
